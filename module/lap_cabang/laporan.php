...
$objPHPExcel->getProperties()->setCreator("Fauzan Azis")
							 ...
							 ->setDescription("Laporan Data Cabang Kantor Camat Simpang Mamplam - Bireuen")
							 ->setKeywords("Data Cabang")
							 ->setCategory("Data Cabang");
...
$objPHPExcel->getActiveSheet()->setCellValue('A9', "NO")
							  ->setCellValue('B9', "KODE CABANG")
							  ->setCellValue('C9', "NAMA CABANG")
							  ->setCellValue('D9', "ALAMAT")
							  ->setCellValue('E9', "PROPINSI")
							  ->setCellValue('F9', "KABUPATEN")
							  ->setCellValue('G9', "TELEPON")
							  ->setCellValue('H9', "PIMPINAN");
...
// query database
$SQL = mysql_query("select * from cabang order by kode_cabang");
...
while($row = mysql_fetch_array($SQL, MYSQL_ASSOC)){
	$no++;
	$row_array['no'] 		  		= $no;
	$row_array['kode_cabang'] 	  	= $row['kode_cabang'];
	$row_array['nm_cabang']    	  	= $row['nm_cabang'];
	$row_array['alamat'] 	  		= $row['alamat'];
	$row_array['propinsi'] 			= $row['propinsi'];
	$row_array['kabupaten'] 	 	= $row['kabupaten'];
	$row_array['telepon'] 	  		= $row['telepon'];
	$row_array['pincab']  	  		= $row['pincab'];
...
// Set fills
$objPHPExcel->getActiveSheet()->getStyle('A9:H9')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A9:H9')->getFill()->getStartColor()->setARGB('FF808080');
...
$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A9:H$nox");
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', "DAFTAR DATA CABANG");
...
$objPHPExcel->getActiveSheet()->getStyle('A2:H5')->getFont()->setName('Arial');
$objPHPExcel->getActiveSheet()->getStyle('A2:H5')->getFont()->setSize(14);
$objPHPExcel->getActiveSheet()->getStyle('A2:H5')->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->getStyle('A3:H5')->getFont()->setSize(13);
...
header('Content-Disposition: attachment;filename="DATA_CABANG"'.date("d-F-Y").'".xlsx"');
