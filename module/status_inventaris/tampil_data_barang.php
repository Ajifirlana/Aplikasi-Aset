...
$query_rs_data = sprintf("SELECT * FROM tampil_inventarisasi WHERE kode_inventarisasi LIKE %s", GetSQLValueString("%" . $colname_rs_data . "%", "text"));
...
$(document).ready(function() {
	...
	function tambah_barang(ID){
		...
		$.ajax({
			type	: "POST",
			url		: "module/status_inventaris/cari_barang.php",
			data	: "kode="+kode,
			dataType	: "json",
			success	: function(data){
				...
				$("#kode_inventarisasi").val(data.kode_inventarisasi);
				$("#id_inventarisasi").val(data.id_inventarisasi);
				$("#kode_cabang").val(data.kode_cabang);
				$("#status_before").val(data.nm_status);
      ...
      <?php do { ?>
        <tr class="gradeX">
          ...
          <td   width="90"><div align="center">
            <a href="javascript:void(0);" onclick="tambah_barang('<?php echo $row_rs_data['kode_barang']; ?>');"><img src="img/icons/packs/fugue/16x16/tick-red.png" width="16" height="16" alt="Pilih" id="pilih" /> </a>
          </div></td>
